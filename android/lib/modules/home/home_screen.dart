...
import '../helper/chargeHelper.dart';
import '../helper/firehelper.dart';
import 'acceptSheet.dart';
import 'addressSelctionView.dart';
import 'class/homeclass.dart';
import 'requset_view.dart';
...
class _HomeScreenState extends State<HomeScreen>
  ...
  void getCredit() async {
    ...
    DatabaseReference riderCRef = FirebaseDatabase.instance.reference().child(
        'userCredit/$phoneNumber');
    final snapshot = await riderCRef.get();

      print(snapshot.value);
      if (snapshot.value != null) {
        var setStated = int.parse(snapshot.value);
        print(setStated);
  ...
  void setCardInfo() async {
    ...
    SharedPreferences prefs = await SharedPreferences.getInstance();
    DatabaseReference cardRef = FirebaseDatabase.instance.reference().child(
            ...
            createRide: () async {
              ...
              final snapshot = await tokenRef.get();
                ...
                if (snapshot.value != null) {
                  ...
                  helperMthod.sendNotification(token, context, rideRef.key);
                } else {
                  return;
                }
              ...
              rideSubscription =
                  rideRef.onValue.listen((event) async {
                    Map valsnapshot = event.snapshot.value;
